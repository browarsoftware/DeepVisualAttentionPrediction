...
make_dir_with_check('res_results')
for stim_name in stimulus:
    make_dir_with_check('res_results/' + stim_name)
    ...
    for sample_id in (range(int(len(dir_data) / 2))):
        ...
        try:
            if os.path.exists(my_path_original) and os.path.exists(my_path_recon):
                ...
                cv2.imwrite('res_results/' + stimulus[stimulus_id] + '/original' + str(sample_id) + '.png', my_stimulus_2)
                cv2.imwrite('res_results/' + stimulus[stimulus_id] + '/recon' + str(sample_id) + '.png', my_stimulus_1)
        except OSError:
            print('image exists')
        else:
            print('image exist')
