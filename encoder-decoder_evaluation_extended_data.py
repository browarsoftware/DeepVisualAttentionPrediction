...
path_to_checkpoints = 'checkpoint2/' + stimulus[stimulus_id] + '/'
...
for i in range(0, Y_test.shape[0]):
    ...
    recon = (decoded[i] * 255).astype("uint8")
    if np.sum(original) > 0:
        cv2.imwrite('res/' + stimulus[stimulus_id] + "/original" + str(i) + ".png", original)
        cv2.imwrite('res/' + stimulus[stimulus_id] + "/recon" + str(i) + ".png", recon)
