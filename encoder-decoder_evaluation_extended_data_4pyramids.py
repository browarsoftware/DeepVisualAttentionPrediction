...
make_dir_with_check('res_4pyramids')
make_dir_with_check('checkpoint_4pyramids')
for stim_name in stimulus:
    make_dir_with_check('data/' + stim_name)
    make_dir_with_check('res_4pyramids/' + stim_name)
    make_dir_with_check('checkpoint_4pyramids/' + stim_name)
...
def decoder2():
    ...
    x = my_model.get_layer("block2_pool").output
    for f in filters[::-1]:
        # apply a CONV_TRANSPOSE => RELU => BN operation
        x = Conv2DTranspose(f, (3, 3), strides=2,
                            padding="same")(x)
    ...
    x = Conv2DTranspose(depth_out, (3, 3), padding="same", name='decoder21_out')(x)
    outputs = Activation("sigmoid", name='decoder2_out')(x)

    decoder = Model(my_model.inputs, outputs, name="decoder")
    return decoder

def decoder2345():
    attention = Concatenate()([d2.output, d3.output, d4.output, d5.output])
...
path_to_checkpoints = 'checkpoint_4pyramids/' + stimulus[stimulus_id] + '/'
...
for i in range(0, Y_test.shape[0]):
    ...
    recon = (decoded[i] * 255).astype("uint8")
    if np.sum(original) > 0:
        cv2.imwrite('res_4pyramids/' + stimulus[stimulus_id] + "/original" + str(i) + ".png", original)
        cv2.imwrite('res_4pyramids/' + stimulus[stimulus_id] + "/recon" + str(i) + ".png", recon)
