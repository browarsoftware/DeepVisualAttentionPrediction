...
make_dir_with_check('res_3pyramids')
make_dir_with_check('checkpoint_3pyramids')
for stim_name in stimulus:
    make_dir_with_check('data/' + stim_name)
    make_dir_with_check('res_3pyramids/' + stim_name)
    make_dir_with_check('checkpoint_3pyramids/' + stim_name)
...
def decoder345():
    attention = Concatenate()([d3.output, d4.output, d5.output])
...
path_to_checkpoints = 'checkpoint_3pyramids/' + stimulus[stimulus_id] + '/'
...
for i in range(0, Y_test.shape[0]):
    ...
    recon = (decoded[i] * 255).astype("uint8")
    if np.sum(original) > 0:
        cv2.imwrite('res_3pyramids/' + stimulus[stimulus_id] + "/original" + str(i) + ".png", original)
        cv2.imwrite('res_3pyramids/' + stimulus[stimulus_id] + "/recon" + str(i) + ".png", recon)
